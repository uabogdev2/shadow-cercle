...
<template>
  <div class="h-screen w-screen bg-black flex flex-col p-4 relative overflow-hidden" :class="phaseClasses">
    ...
    <!-- Header -->
    <header class="relative z-10 flex justify-between items-center border-b-2 border-white pb-4 mb-4">
      <div class="flex items-center gap-4">
        <component :is="roleIcon" class="w-8 h-8" :style="{ color: roleColor }" />
        <div>
          <p class="text-mono text-xs uppercase text-gray-500">NIGHT SEQUENCE {{ gameStore.currentGame?.day_number }}</p>
          <p class="text-display text-xl uppercase" :style="{ color: roleColor }">{{ getPhaseTitle() }}</p>
        </div>
      </div>
    ...
    <main class="relative z-10 flex-1 overflow-y-auto">
      <div v-if="gameStore.canAct" class="grid grid-cols-2 gap-4">
        <PlayerCard
      ...
      <div v-else class="flex flex-col items-center justify-center h-full border-2 border-dashed border-gray-700 p-8">
        <p class="text-display text-2xl text-gray-500 mb-2">STANDBY MODE</p>
        <p class="text-mono text-xs text-gray-600 uppercase">Waiting for other agents...</p>
      </div>
    </main>

    <!-- Footer -->
    <footer class="relative z-10 pt-4 border-t-2 border-white mt-auto">
      <ActionButton
    ...
    <div v-if="isWerewolf" class="absolute bottom-20 left-4 right-4 z-20 h-32 opacity-90 hover:opacity-100 transition-opacity">
      <ChatBox channel="wolves" :is-visible="true" class="h-full shadow-lg" />
    </div>
  </div>
</template>
...
const roleConfig = {
  werewolf: { 
    icon: Moon, color: '#ef4444', actionText: "SELECT TARGET TO ELIMINATE", selectionColor: 'red-600', buttonVariant: 'danger', buttonText: 'DEVOUR'
  },
  seer: { 
    icon: Eye, color: '#a855f7', actionText: "SELECT TARGET TO SCAN", selectionColor: 'purple-500', buttonVariant: 'magic', buttonText: 'REVEAL'
  },
  guard: { 
    icon: Shield, color: '#3b82f6', actionText: "SELECT TARGET TO PROTECT", selectionColor: 'blue-500', buttonVariant: 'primary', buttonText: 'SECURE'
  },
  hunter: {
    icon: Target, color: '#f59e0b', actionText: "SELECT TARGET TO ELIMINATE", selectionColor: 'red-600', buttonVariant: 'danger', buttonText: 'FIRE'
  }
};
...
const roleActionText = computed(() => config.value.actionText || "NIGHT PROTOCOL INITIATED");
...
const phaseClasses = computed(() => isWerewolf.value ? 'border-red-900' : 'border-gray-800');
